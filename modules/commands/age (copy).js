...
module.exports.run = async function ({ event, args, api }) {
  ...
  var input = args[0];
  if (!input) return api.sendMessage(`enter correct format >age [date/month/years]`,event.threadID,event.messageID);
  var cc = input.split("/");
  var ngay1 = parseInt(cc[0]);
  if (!ngay1 || isNaN(ngay1) || ngay1 > 31 || ngay1 < 1) return api.sendMessage("wrong format",event.threadID,event.messageID);
  var thang1 = parseInt(cc[1]);
  if (!thang1 || isNaN(thang1) || thang1 > 12 || thang1 < 1) return api.sendMessage("Plz enter the sae format",event.threadID,event.messageID);
  var nam1 = parseInt(cc[2]);
  if (!nam1) return api.sendMessage("[❌]➜ Enter the sae format!",event.threadID,event.messageID);
  ...
  return api.sendMessage({body: `====「 AGE CALCULATOR 」====\n\n➜ Your Date of Birth is: ${input}\n➜ -Nnumber of years passed: ${nam} \n➜ Number of months passed: ${thang} \n➜ Number of weeks passed: ${tuan} \n➜ Number of days passed: ${ngay} \n➜ Hours passed: ${gio} ⏰\n➜ Mintues passed: ${phut} 💗\n➜ The number of seconds passed: ${giay} 🍁\n━━━━━━━━━━━━━━━━━━\n➜ Now your age is ${nam1}🌐`, attachment: await streamURL(`https://graph.facebook.com/${event.senderID}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`)},event.threadID,event.messageID);
}