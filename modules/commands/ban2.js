module.exports.config = {
	...
	description: "Banned User & Kicked From Group",
	commandCategory: "Admin - Tools",
	usePrefix: "True - ✅",
	usages: `${global.config.PREFIX}banv2 [ Mention ]\n\n» Example : ${global.config.PREFIX}banv2 @KHAN BOT`,
	cooldowns: 5,
	info: [
		{
	...
	*/
	...
	if(args[0] == "view") {
		if(!args[1]) {
			...
			api.sendMessage(`You have been warned for the reason : ${msg} - ⚠️`, threadID, messageID);
		}
			...
			allwarn == "" ? api.sendMessage("No one in your group has been warned", threadID, messageID) : api.sendMessage("⚠️ - Banned List - ⚠️\n\n"+allwarn, threadID, messageID);
		}
	}
	...
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎Right cunt border!', threadID, messageID);
		...
		if(!id) return api.sendMessage("Enter the uid of the person to be removed from the banned list of the group - ⚠️", threadID, messageID);
		bans.banned;
		if(!mybox.includes(id)) return api.sendMessage("This person hasn't been banned from this group - ⚠️", threadID, messageID);
			api.sendMessage(`Removed the member ${id} from the group banned list - ✅`, threadID, messageID);
		...
		msg == "" ? api.sendMessage("No one in this group has been - ⚠️", threadID, messageID) : api.sendMessage("⚠️ Banned List Of This Group ⚠️\n\n"+msg, threadID, messageID);
	}
	...
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎Right cunt border!', threadID, messageID);
		...
		fs.writeFileSync(__dirname + `/cache/bans.json`, JSON.stringify(bans, null, 2));
		api.sendMessage("Reset All Data Of This Group", threadID, messageID);
	}
		 //◆━━━━━━━━━◆WARN◆━━━━━━━━━◆\\
		 else{ 
	...
	var reason = "";
		...
		api.sendMessage({body: `Banned members ${arrayname.join(", ")} permanently leave from the group - ⚠️\n\nBanned Reason : ${reason}`, mentions: arraytag}, threadID, messageID);
		fs.writeFileSync(__dirname + `/cache/bans.json`, JSON.stringify(bans, null, 2));
}
