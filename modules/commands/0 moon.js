...
module.exports.handleReply = async ({ api, event, args, handleReply }) => {
  ...
  switch (type) {
    case 'replyDay': {
      ...
      if (day < 1 || day > 31)
        return api.sendMessage("The date you entered is not valid", author);
      date.day = day;
      return api.sendMessage("Reply to this message and enter the month", event.threadID, (error, info) => {
    ...
    case 'replyMonth': {
      ...
      if (month < 1 || month > 12)
        return api.sendMessage("The month you entered is not valid", author);
      date.month = month;
      return api.sendMessage("Reply to this message and enter the year", event.threadID, (error, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author,
    ...
    default: {
      try {
        ...
        if (!date2)
          return message.reply(`Please enter a valid date/month/year in the format DD/MM/YYYY`);
        ...
        try {
          ...
          return message.reply(`An error occurred that could not get the moon photo of the day ${args[0]}`);
        }
        ...
        const msg = `- Moon photo at night ${`${date.day}/${date.month}/${event.body}`}`
          + `\n- ${$($('h3').get()[0]).text()}`
          + `\n- ${$("#phimg > small").text()}`
          + `\n- ${linkCrawl}`
...
module.exports.run = async ({ api, event }) => {
  api.sendMessage("Reply to this message and enter the date", event.threadID, (err, info) => {
    global.client.handleReply.push({
      name: this.config.name,
      messageID: info.messageID,
