...
module.exports.run = async function({
  ...
  try {
    ...
    case "-i":{
      ...
      if (!username) return api.sendMessage("id người dùng đâu", threadID);
       const res = await axios.get(`https://api.samirthakuri.repl.co/api/iginfo?username=${username}`);
...
const res = await axios.get(`https://caochungdat.me/docs/instagram/videodl?url=${link}`);
var url = res.data.images[0].image_versions2.candidates[0].url;
    ...
    data = (await axios.get(`https://api.samirthakuri.repl.co/api/instadl?url=${link_video}`)).data,
    ...
    .catch(error=>(console.log(error), api.sendMessage(error.message, event.threadID))); // gửi lỗi nếu request đến API thất bại
        ...
        default: api.sendMessage(`sử dụng\n==> -v ( video )
==> -m ( music )
==> -i ( info )
==> -p ( ảnh )\nđể tìm kiếm`, threadID)
    }
